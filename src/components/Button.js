...
export default function Button({ title, onPress, image, isize, style, textStyle, pressAble, colorScheme }) {
  return (
    pressAble ? (
      <Pressable android_ripple={{color:colorScheme=='dark' ? '#fff' : '#2473FE', borderless: true}} className={`flex-row items-center justify-center h-10 ${style ? style : ''}`} onPress={onPress} >
